...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="shrink-0 h-14 border-b bg-card shadow-sm">
        <div className="max-w-[1200px] mx-auto px-4 md:px-6 h-full flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="hidden sm:block text-base font-bold tracking-tight">여행 지출 관리</h1>
          </div>

          <div className="flex items-center gap-2">
            <Select value={currency} onValueChange={v => setCurrency(v as CurrencyCode)}>
              <SelectTrigger className="w-28 sm:w-48 h-8 text-xs" aria-label="통화 선택">
                <SelectValue>
      ...
      {/* Main: single column on mobile, two-column on desktop */}
      <main className="flex-1 md:overflow-hidden py-4 md:py-5">
        <div className="max-w-[1200px] mx-auto px-4 md:px-5 md:h-full">
          <div className="flex flex-col gap-4 md:grid md:grid-cols-[1fr_360px] md:gap-5 md:h-full">
            {/* Left: expense list */}
            <div className="bg-card rounded-xl border shadow-sm flex flex-col md:overflow-hidden">
              <div className="px-5 pt-5 pb-3 shrink-0">
                <h2 className="text-sm font-semibold">지출 목록</h2>
              </div>
              <div className="flex-1 md:overflow-hidden px-3 pb-4 md:min-h-0">
                <ExpenseList
                  expenses={expenses}
                  currency={currency}
                  onAdd={handleAdd}
